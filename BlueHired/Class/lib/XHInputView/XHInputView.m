...
#define XHInputView_StyleLarge_LRSpace 13
...
-(void)setupUI{
    ...
    switch (_style) {
        ...
        case InputViewStyleLarge:{
            ...
            CGFloat height = LENGTH_SIZE(148);
            
            _inputView.frame = CGRectMake(0, XHInputView_ScreenH, XHInputView_ScreenW, height);
            
            /** StyleLargeUI */
            CGFloat sendButtonWidth = LENGTH_SIZE(58);
            CGFloat sendButtonHeight = LENGTH_SIZE(48);
            _sendButton = [UIButton buttonWithType:UIButtonTypeCustom];
            _sendButton.frame = CGRectMake(XHInputView_ScreenW  - sendButtonWidth,
                                           0,
                                           sendButtonWidth,
                                           sendButtonHeight);
            [_sendButton setTitleColor:[UIColor colorWithHexString:@"#666666"] forState:UIControlStateNormal];
            [_sendButton setTitle:@"发送" forState:UIControlStateNormal];
            _sendButton.titleLabel.font = [UIFont systemFontOfSize:FontSize(15)];
            ...
            [_cancelButton setTitleColor:[UIColor colorWithHexString:@"#939393"] forState:UIControlStateNormal];
            [_cancelButton setTitle:@"取消" forState:UIControlStateNormal];
            _cancelButton.titleLabel.font = [UIFont systemFontOfSize:FontSize(15)];
            _cancelButton.layer.cornerRadius = 1.5;
            [_cancelButton addTarget:self action:@selector(bgViewClick) forControlEvents:UIControlEventTouchUpInside];
            [_inputView addSubview:_cancelButton];
            
            _textBgView = [[UIView alloc] initWithFrame:CGRectMake(XHInputView_StyleLarge_LRSpace,
                                                                   _cancelButton.lx_bottom,
                                                                   XHInputView_ScreenW-2*XHInputView_StyleLarge_LRSpace,
                                                                    height - _cancelButton.lx_bottom - XHInputView_StyleLarge_LRSpace)];
            _textBgView.backgroundColor = [UIColor colorWithHexString:@"#F2F2F2"];
            [_inputView addSubview:_textBgView];
            
            _textView = [[UITextView alloc] initWithFrame:CGRectMake(0,
                                                                     0,
                                                                     _textBgView.bounds.size.width,
                                                                     _textBgView.bounds.size.height-XHInputView_CountLabHeight)];
            _textView.backgroundColor = [UIColor clearColor];
            _textView.font = [UIFont systemFontOfSize:FontSize(15)];
            _textView.delegate = self;
            ...
            _countLab.font = [UIFont systemFontOfSize:FontSize(14)];
