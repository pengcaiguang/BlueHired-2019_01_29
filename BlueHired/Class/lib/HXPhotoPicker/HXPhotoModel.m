...
- (PHImageRequestID)requestAVAssetExportSessionStartRequestICloud:(HXModelStartRequestICloud)startRequestICloud
    ...
    if (self.type == HXPhotoModelMediaTypeCameraVideo) {
        if (@available(iOS 11.0, *)) {
            AVAssetExportSession *export = [AVAssetExportSession exportSessionWithAsset:[AVAsset assetWithURL:self.videoURL] presetName:AVAssetExportPresetMediumQuality];
            if (success) {
    ...
    HXWeakSelf
    if (@available(iOS 11.0, *)) {
        requestId = [[PHImageManager defaultManager] requestExportSessionForVideo:self.asset options:options exportPreset:AVAssetExportPresetMediumQuality resultHandler:^(AVAssetExportSession * _Nullable exportSession, NSDictionary * _Nullable info) {
