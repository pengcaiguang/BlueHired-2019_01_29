...
#define CellImageViewHeight LENGTH_SIZE(200)
#define CellHeight LENGTH_SIZE(246)
...
{
    [super awakeFromNib];
    [self.retryButton setImage:[UIImage imageNamed:@"video_refresh"] forState:UIControlStateNormal];
...
//            return;
    ...
    [self showMaskView];
        ...
        if (self.fullScreen) { //由全屏进入后台 在进入前台 会显示全屏 但是一旋转frame跟全屏一样
           self.layer.cornerRadius = 0;
           self.playerMaskView.VideoName.hidden = NO;
    
           self.playerMaskView.WorkBtn.hidden = self.model.mechanismName.length>0?NO:YES;
           self.playerMaskView.ShareBtn.hidden = NO;
           self.playerMaskView.CommonBtn.hidden = NO;
           self.playerMaskView.WorkBtn2.hidden = YES;

 
       } else {
           self.layer.cornerRadius = LENGTH_SIZE(6);
           self.playerMaskView.VideoName.hidden = YES;
           self.playerMaskView.WorkBtn.hidden = YES;
           self.playerMaskView.ShareBtn.hidden = YES;
           self.playerMaskView.CommonBtn.hidden = YES;
           self.playerMaskView.WorkBtn2.hidden = self.model.mechanismName.length>0?NO:YES;
    ...
    //第一次才添加
    if (self.superview == nil || self.superview != self.tempSuperView ) {
    ...
    if (self.playerMaskView == nil) {
        ...
        //关闭按钮点击事件
        [self.playerMaskView.closeButton addTarget:self action:@selector(clickFullScreen) forControlEvents:UIControlEventTouchUpInside];
    ...
    self.playerMaskView.model = self.model;
...
- (void)playOrPause:(UIButton*)sender
{
    ...
    if (self.playerStatus == LSPlayerStatusPlaying) {
        ...
        NSInteger type = [NetworkReachabilityManager sharedInstance].netType;
...
- (void)cancelPreviousPerformAndHideMaskView
{
    ...
    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(nowHideMaskView) object:nil];
    if (self.locationType != LSPlayerViewLocationTypeBottom) {
        [self performSelector:@selector(nowHideMaskView) withObject:nil afterDelay:3];
    }
}
...
- (void)moviePlayDidEnd:(NSNotification*)note
{
    ...
    self.playerMaskView.slider.value =  1; //当前进度
...
//    NSLog(@"self.currentFrame.origin.y = %f  y=%f       self.tempSuperView.frame.size.height = %f       top=%f",self.currentFrame.origin.y,y,self.tempSuperView.frame.size.height,top);
    if (self.currentFrame.origin.y - y > self.tempSuperView.frame.size.height  - top) { //在底下
...
//        self.playerStatus = LSPlayerStatusPause;
        ...
        [self closeClick];
    } else if (y - CellHeight > rect.origin.y) {
        //在上面
        [self closeClick];
...
{
    if (self.isLoseActive ||!kAppDelegate.allowRotation )
        return;
