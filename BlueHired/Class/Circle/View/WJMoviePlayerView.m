...
-(void)longPress:(UILongPressGestureRecognizer *)sender{
    ...
    if (sender.state == UIGestureRecognizerStateBegan) {
        ...
        UIAlertAction *action1 = [UIAlertAction actionWithTitle:@"保存视频" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            [self saveVideo:[self.playerURL path]];
        }];
...
- (void)video:(NSString *)videoPath didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    if (error) {
        ...
        [LPTools AlertMessageView:@"保存视频失败" dismiss:1.0];
    }
    else {
        [LPTools AlertMessageView:@"保存视频成功" dismiss:1.0];
    }
...
- (UIView *)playerView {
    if (!_playerView) {
        ...
        [_playerView addGestureRecognizer:longPress];
    }
    return _playerView;
}
