...
#import "DSBeiAnimationLoading.h"

static CGFloat ballScale = 1.5f;
@interface DSBeiAnimationLoading()<CAAnimationDelegate>
{
...
@end
@implementation DSBeiAnimationLoading


-(instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        self.backgroundColor = SetColor(240, 240, 240, 0.5);
        [self initUI];
    }
    return self;
}
...
-(void)initUI{
    ...
    _ball2.backgroundColor = GreenColor;
...
+(void)showInView:(UIView *)view{
    ...
    DSBeiAnimationLoading *loading = [[DSBeiAnimationLoading alloc] initWithFrame:view.bounds];
    [view addSubview:loading];
    [loading start];
}

+(void)hideInView:(UIView *)view{
    for (DSBeiAnimationLoading *loading in view.subviews) {
        if ([loading isKindOfClass:[DSBeiAnimationLoading class]]) {
            [loading stop];
