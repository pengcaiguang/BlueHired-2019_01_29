...
@interface LPLendAuditScreen ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation LPLendAuditScreen
-(instancetype)init{
    self = [super init];
    if (self) {
        self.titleArray = @[@"全部",@"未审核",@"已通过"];
        self.selectTitle = @"全部";
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(cell == nil){
        ...
        UIImageView *imageViews = [[UIImageView alloc] initWithFrame:CGRectMake( SCREEN_WIDTH/3*2 - 40, 15, 16, 13)];
